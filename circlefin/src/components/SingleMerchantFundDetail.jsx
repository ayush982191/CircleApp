...
const SingleMerchantFundDetail = () => {
  const {id} = useParams();


  // console.log("id="+id);
  const farmerFund = merchantFundData.find((fund)=>fund.id == id);
  const { "Fund Name":fundName,"Target Return":targetReturns,"Risk Level":riskLevel,"Minimum Investment":minimumInvestment, "Investment Period":investmentPeriod,"Detailed Description":detailedDesc } = farmerFund;   
  const { "Initial Labor Costs":initialLabourCost,"Investment Focus":investmentFocus,Fertilizers,Seeds, "Fund Size":fundSize,"Why Invest":whyInvest} = detailedDesc;              
    ...
    <div className="container mx-auto mt-32 px-4">
      <div className="flex flex-col lg:flex-row justify-around items-start lg:items-center space-y-8 lg:space-y-0 lg:space-x-8">
        <div className="flex-shrink-0">
          <img src={merchantFundImages[id]} className="w-full lg:w-[30rem] h-auto lg:h-[20rem]" alt="" />
...
export default SingleMerchantFundDetail